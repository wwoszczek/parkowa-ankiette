...
`src/ui/styles.py` injects the global stylesheet (Manrope font, `pk-*` component classes); `src/ui/components.py` renders presentational HTML (hero card, players table, team cards, stat chips, podium, ranking list, empty states) — always `html.escape` user content there. `src/ui/graphics.py` provides self-contained inline SVG helpers (`ball`, `jersey`, `HERO_DECO`, `empty_illustration`) — no external requests, safe on free tier. Buttons are styled per-widget via `st-key-<key>` CSS classes (e.g. keys starting with `remove` get the destructive red style, `login_google` gets the branded Google style). Avoid emoji; use `:material/...:` icons.
...
- Team drawing (`src/utils/team_utils.py:draw_teams`) accepts an optional `pins` dict `{color: [players]}` to fix specific players to a shirt colour before the random shuffle — for players who only own one kit colour. The `/sklady` page exposes this via a collapsible expander before the draw button.
